...
export(float, 0, 1) var steepness = 0.3 setget set_steepness
export(float, 0, 10000) var amplitude = 0.5 setget set_amplitude
export(Vector2) var wind_direction = Vector2(1, 0) setget set_wind_direction
export(float, 0, 1) var wind_align = 1.0 setget set_wind_align
export(float) var speed = 10.0 setget set_speed

export(bool) var noise_enabled = true setget set_noise_enabled
export(float) var noise_amplitude = 1 setget set_noise_amplitude
export(float) var noise_frequency = 0.035 setget set_noise_frequency
...
var counter = 0.5
var cube_cam = preload("res://Water/CubeCamera.tscn")
var cube_cam_inst;
...
func _process(delta):
	...
	material_override.set_shader_param('time_offset', OS.get_ticks_msec()/1000.0 * speed)
		...
		#set_noise_enabled(true)
		#set_noise_amplitude(1)
		#set_noise_frequency(0.025)
		#set_noise_speed(0.4)
...
func get_displace(position):
	...
	for i in waves:
		...
		# Cancel steepness force
		steep = 0.1 / (w*amp)
		
		new_p.x += steep*amp * dir.x * cos(W) *.02
		new_p.z += steep*amp * dir.y * cos(W) *.02
		new_p.y += amp * sin(W)
	return new_p;
